...
export function KivoHistoryScreen() {
  ...
  function ConversationItem({ conversation }: { conversation: ConversationRow }) {
    ...
    function onPointerUp() {
      ...
      if (dragX < -62) {
        setDragX(-82);
        window.setTimeout(() => deleteConversation(conversation.id), 120);
      } else {
        setDragX(0);
      }
    }
    ...
    return (
      <div className="relative overflow-hidden border-b border-black/[0.045] last:border-b-0">
        <div className="absolute inset-y-0 right-0 flex w-[86px] items-center justify-center bg-[#f4f4f5] text-[#202024]">
        ...
        <div
          ...
          className="relative flex min-h-[76px] items-center gap-[13px] bg-white/82 px-[14px] py-[11px] transition-transform duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] active:scale-[0.997]"
              ...
              <button type="button" onClick={() => openConversation(conversation.id)} className="flex min-w-0 flex-1 items-center gap-[13px] text-left">
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-[7px]">
                    {conversation.is_favorite ? <Star size={13} className="shrink-0 fill-[#202024] text-[#202024]" /> : null}
                    <span className="truncate text-[15.5px] font-semibold tracking-[-0.035em] text-[#15161a]">{conversation.title || 'New conversation'}</span>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f3f3f5] text-[#202024]">
      ...
      <div className="relative z-10 mx-auto min-h-screen w-full max-w-[430px] px-[28px] pb-[40px] pt-[126px]">
        ...
        <label className="mt-[28px] flex h-[48px] items-center gap-[11px] rounded-[24px] bg-white/86 px-[15px] shadow-[0_14px_34px_rgba(15,23,42,0.032)] ring-1 ring-black/[0.025] backdrop-blur-xl">
          <Search size={19} strokeWidth={1.9} className="text-[#8b8d94]" />
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversations..." className="min-w-0 flex-1 bg-transparent text-[15px] tracking-[-0.025em] text-[#202024] outline-none placeholder:text-[#9b9ca3]" />
        </label>

        {loading ? <div className="mt-[34px] space-y-[12px]">{[0, 1, 2].map((item) => <div key={item} className="h-[76px] animate-pulse rounded-[24px] bg-white/70" />)}</div> : groups.length ? (
          <div className="mt-[30px] space-y-[28px]">
            {groups.map((group) => <section key={group.title}><h2 className="mb-[10px] text-[15px] font-medium tracking-[-0.03em] text-[#73747b]">{group.title}</h2><div className="overflow-hidden rounded-[24px] bg-white/82 shadow-[0_14px_38px_rgba(15,23,42,0.04)] ring-1 ring-black/[0.025] backdrop-blur-xl">{group.items.map((conversation) => <ConversationItem key={conversation.id} conversation={conversation} />)}</div></section>)}
          </div>
        ) : <div className="mt-[44px] rounded-[28px] bg-white/76 px-[22px] py-[28px] text-center shadow-[0_14px_38px_rgba(15,23,42,0.035)] ring-1 ring-black/[0.025]"><div className="mx-auto flex h-[46px] w-[46px] items-center justify-center rounded-[16px] bg-[#f2f2f3] text-[#202024]"><Clock3 size={22} strokeWidth={1.9} /></div><h2 className="mt-[14px] text-[18px] font-semibold tracking-[-0.04em] text-[#15161a]">No conversations found</h2><p className="mt-[6px] text-[13.5px] leading-[1.35] tracking-[-0.025em] text-[#777981]">Your previous Kivo conversations will appear here.</p></div>}
        <p className="mt-[26px] text-center text-[12.5px] tracking-[-0.02em] text-[#8c8e96]">Conversations are private and secure.</p>
      </div>

      {actionMenu && selectedConversation ? (
        <div className="fixed inset-0 z-[80]">
          <button type="button" aria-label="Close actions" onClick={() => setActionMenu(null)} className="absolute inset-0 bg-black/[0.06] backdrop-blur-[1.5px]" />
          <div className="absolute inset-x-[22px] bottom-[34px] mx-auto max-w-[386px] overflow-hidden rounded-[28px] bg-[#fbfbfc] shadow-[0_18px_54px_rgba(15,23,42,0.14)] ring-1 ring-black/[0.045]">
            <div className="px-[18px] py-[16px]"><div className="truncate text-[16px] font-semibold tracking-[-0.04em] text-[#15161a]">{actionMenu.title}</div><div className="mt-[3px] text-[12px] tracking-[-0.02em] text-[#7a7c84]">Conversation actions</div></div>
            <div className="border-t border-black/[0.05]">
              <button type="button" onClick={() => toggleFavorite(actionMenu.id, !actionMenu.isFavorite)} className="flex h-[54px] w-full items-center gap-[13px] px-[18px] text-left text-[15px] font-medium tracking-[-0.03em]"><Star size={19} className={actionMenu.isFavorite ? 'fill-[#202024]' : ''} />{actionMenu.isFavorite ? 'Remove from pinned' : 'Pin conversation'}</button>
              <button type="button" onClick={() => startRename(selectedConversation)} className="flex h-[54px] w-full items-center gap-[13px] border-t border-black/[0.04] px-[18px] text-left text-[15px] font-medium tracking-[-0.03em]"><Pencil size={19} />Rename</button>
              <button type="button" onClick={() => deleteConversation(actionMenu.id)} className="flex h-[54px] w-full items-center gap-[13px] border-t border-black/[0.04] px-[18px] text-left text-[15px] font-medium tracking-[-0.03em] text-[#202024]"><Trash2 size={19} />Delete</button>
            </div>
          </div>
        </div>
      ) : null}
